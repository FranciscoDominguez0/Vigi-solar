  ...
  return (
    <div className={`bg-[#1a1a1a] border border-gray-800 rounded-2xl overflow-hidden hover:border-accent transition-colors duration-500 group flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} h-full items-stretch`}>
      <div className="w-full md:w-2/5">
        <img src={imagenSrc} alt={`${titulo} (${potencia})`} className="w-full h-auto md:h-full object-cover" />
      </div>
      <div className="p-8 flex flex-col w-full md:w-3/5">
        ...
        <Link href={`/cotizacion/?kit=${kitQuery}`} className="mt-auto w-full md:w-auto self-start text-center bg-gray-800 text-white px-8 py-3 rounded-xl font-semibold group-hover:bg-accent group-hover:text-white transition-all duration-300 shadow-lg">
          Cotizar este Kit
        </Link>
      </div>
    </div>
